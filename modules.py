...
class SuperDrawQLabel(QLabel):
    def __init__(self, parent, cache_dir, printer, background_file="Icons/black.jpg"):
        super().__init__(parent)
        ...
        self.cacheOutputDir = os.path.join(cache_dir, "Outputs")
    ...
    def _updateBaseSeg(self, pre_idx, cur_idx, draw_file, cur_file):
        ...
        seg = deepcopy(np_seg)
        seg[seg == 100] = 0
        seg[seg != label_to_id[self.showLabel]] = 0
        seg = cmap[seg]
        self.printer(seg.shape, "++++++++++++++++++++++++++")
        ...
        cv2.imwrite(os.path.join(self.cacheOutputDir, "show_seg.jpg"), new_seg)
        ...
        image = cv2.addWeighted(image, self.alpha, seg, 1.0-self.alpha, 0)
        cv2.imwrite(cur_file, image)
    ...
    def mouseReleaseEvent(self, event):
        if not self.openPaint:
            ...
            if event.button() == Qt.LeftButton:
                ...
                self._updateBaseSeg(self.pre_idx, self.cur_idx, draw_file, cur_file)
                shutil.copy(cur_file, self.base_mask_with_img_file)
                self.setPixmap(cur_file)
